...
class Pipeline (xlen: Int, memSize: Int, csrFileSize: Int) extends Module {
  val io = IO(new Bundle {
    ...
    val debugIsBranch = Output(Bool())
    val debugIsJump = Output(Bool())
    val debugBranchStallOut = Output(Bool())
    val debugLoadStallOut = Output(Bool())
  })
  ...
  val CSRFile = Module(new CSRFile(csrFileSize, xlen)); 
  val idLogic = Module(new IDControlLogic(xlen)); 
  val branchHazardUnit = Module(new BranchHazardUnit); 
  val forwardingUnit = Module(new ForwardingUnit(xlen)); 
  val loadHazardUnit = Module(new LoadHazardUnit(xlen)); 
  ...
  branchHazardUnit.io.IDEXIsBranch := IDExRegister.io.isBranch
  ...
  // debug stuff
  io.pcOut := pc.io.pc
  io.debugAluResult := alu.io.out
  io.debugRegs := regfile.io.debugRegs
  io.debugCsr := CSRFile.io.debugCsr
  io.debugBranchStallOut := branchHazardUnit.io.branchStall
  io.debugLoadStallOut := loadHazardUnit.io.loadStall
  io.debugIsJump := IDExRegister.io.isJump
  io.debugIsBranch := IDExRegister.io.isBranch
}
